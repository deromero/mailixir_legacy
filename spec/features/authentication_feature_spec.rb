...
RSpec.describe 'user authentication' do
  ...
  it 'does not allow signin whit invalid credentials' do
    sign_in(user, subdomain: account.subdomain,password: 'wrong password')
  ...
  it 'dows not allow user from one subdomain to sign in on another subdomain' do
    user2 = build(:user)
    account2 = create(:account_with_schema, owner: user2)

    sign_in(user2, subdomain: account2.subdomain)
    expect(page).to have_content('Signed in successfully')

    sign_in(user2, subdomain: account.subdomain)
    expect(page).to have_content('Invalid Email or password')
